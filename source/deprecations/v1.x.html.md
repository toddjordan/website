...
As of 1.13.2 you should not use `key=@guid` or `key=@item` in the `{{each}}` helper. Prior to that in 1.13, it was
[documented](http://guides.emberjs.com/v1.13.0/templates/displaying-a-list-of-items/#toc_specifying-keys)
that you could use these special values to uniquely identify each array entry so that the view code could
more efficiently re-render the display of the list. This process was simplified as part of a
[bugfix](https://github.com/emberjs/ember.js/pull/11461), where the key implicitly defaults to `@identity`,
which will pick either a guid or an item identifier based on type of each array entry. Providing the key
attribute is no longer required for re-render optimizations.
